...
test("Case 12b: バージョン不一致ペイロードは破損扱いで復旧する", async ({ page }) => {
  await page.addInitScript((key) => {
    window.localStorage.setItem(key, JSON.stringify({ version: 999, books: [], progressLogs: [] }));
  }, STORAGE_KEY);
...
test("Case 12c: books フィールドが配列でないペイロードは破損扱いで復旧する", async ({ page }) => {
  await page.addInitScript((key) => {
...
test("Case 18b: stats ページに登録書籍数と完読率が正しく表示される", async ({ page }) => {
  ...
  await expect(page.getByTestId("stats-page")).toBeVisible();
  // 登録書籍数 KPI（data-testid なし → .stats-kpi-books article 内のテキストで特定）
  await expect(page.locator("article.stats-kpi-books").getByText("3")).toBeVisible();
  // 完読率 KPI = 66.7%
  await expect(page.locator("article.stats-kpi-completed").getByText("66.7%")).toBeVisible();
});
